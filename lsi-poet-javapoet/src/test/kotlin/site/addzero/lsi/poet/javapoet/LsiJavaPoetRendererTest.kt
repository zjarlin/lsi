...
class LsiJavaPoetRendererTest {
    ...
    fun `rejects Kotlin only declaration names and explicit imports`() {
        ...
        val importedArtifact = LsiSourceArtifact(
            file = LsiFile(
                ...
                members = listOf(LsiClass("Imported", LsiTypeDeclarationKind.CLASS)),
            ),
            typeNames = emptyList(),
            aggregationMode = ArtifactAggregationMode.ISOLATING,
            originatingSymbols = setOf(LsiSymbolId.type("demo.Source")),
    ...
    private fun artifact(
        ...
        val type = if (member is LsiClass) member else {
            LsiClass(fileName, LsiTypeDeclarationKind.CLASS, members = listOf(member))
        }
