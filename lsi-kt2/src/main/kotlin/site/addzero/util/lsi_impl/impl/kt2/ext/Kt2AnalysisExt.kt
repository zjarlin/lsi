package site.addzero.util.lsi_impl.impl.kt2.ext
...
fun KtClass.toLsiClassK2(): LsiClass {
    return analyze(this) {
        ...
        if (symbol is KaClassSymbol) {
            Kt2LsiClass(this@toLsiClassK2, symbol, this)
        } else {
...
fun KtProperty.toLsiFieldK2(): LsiField {
    return analyze(this) {
        ...
        if (symbol is KaPropertySymbol) {
            Kt2LsiField(this@toLsiFieldK2, symbol, this)
        } else {
 ...
 */
inline fun <T> KtClass.withK2Analysis(crossinline block: KaSession.(Kt2LsiClass) -> T): T {
    return analyze(this) {
        val symbol = this@withK2Analysis.symbol
        if (symbol is KaClassSymbol) {
            val lsiClass = Kt2LsiClass(this@withK2Analysis, symbol, this)
            block(lsiClass)
        } else {
            throw IllegalStateException("Expected KaClassSymbol but got ${symbol::class.simpleName}")
        }
 ...
 * 在 analyze 块中执行操作，提供 KaSession 上下文
 */
inline fun <T> KtProperty.withK2Analysis(crossinline block: KaSession.(Kt2LsiField) -> T): T {
    return analyze(this) {
        val symbol = this@withK2Analysis.symbol
        if (symbol is KaPropertySymbol) {
            val lsiField = Kt2LsiField(this@withK2Analysis, symbol, this)
            block(lsiField)
        } else {
            throw IllegalStateException("Expected KaPropertySymbol but got ${symbol::class.simpleName}")
        }
...
fun List<KtClass>.toLsiClassesK2(): List<LsiClass> {
    ...
    return analyze(first()) {
        this@toLsiClassesK2.mapNotNull { ktClass ->
            ...
            if (symbol is KaClassSymbol) {
                Kt2LsiClass(ktClass, symbol, this)
            } else null
        }
    }
}
